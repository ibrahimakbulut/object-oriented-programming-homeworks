...
/*if(Container_shape.getter_pos_x()<0){

	cout<<

}
	Container_shape.setter_position(0,Container_shape.getter_pos_y());
}
if(Container_shape.getter_pos_y()<0)
	Container_shape.setter_position(Container_shape.getter_pos_x(),0);   //

if(inner_shape.getter_pos_x()<0)
	inner_shape.setter_position(0,inner_shape.getter_pos_y());
if(inner_shape.getter_pos_y()<0)
	inner_shape.setter_position(inner_shape.getter_pos_x(),0);*/

Polygon temp_Object(Container_shape);
Polygon temp_Object1(inner_small_shape);
...
composedShape::composedShape(){ //otomatically Polygon objects created by their no parameter constructor and inside Polygon objects
								//polygon_points point to nullptr ,size_of_polygon points equal 0.
	
}
composedShape& composedShape::operator +=(Polygon shape){ //this operator overload adds a shape as Polygon to composedShape object.
	inner_shapes_vector.push_back(shape);
	return *this;
}
const Polygon& composedShape::operator [](unsigned int index)const{ //with this operator an element of vector is only accessable, not changeable
	if(inner_shapes_vector.size()==0){
		cout<<"vector is empty, can not be reached an element of it"<<endl;
		Polygon b; //if vector is empty then return a shape element that has null as void pointer variable.
		Polygon& a=b;
		return a;
	}
	if(index<0 || index>inner_shapes_vector.size()){
		cout<<"vector  index out of range."<<endl;
		Polygon b; //if index is wrong then return the first element of vector.
		Polygon& a=b;
		return a;
	}
...
ofstream& operator <<(ofstream& outputfile,const composedShape& main_shape){ //for writing a composedshape 
	...
	if( (main_shape.container_shape.get_size()==3 || main_shape.container_shape.get_size()==4 || main_shape.container_shape.get_size()==100) && 
		(main_shape.inner_shape.get_size()==3 || main_shape.inner_shape.get_size()==4 || main_shape.inner_shape.get_size()==100) ){

	double most_right_x,most_down_y;
	most_right_x=main_shape.container_shape.most_x();
	most_down_y=main_shape.container_shape.most_y();
	...
	for(auto i=0;i<main_shape.container_shape.get_size();++i){
		outputfile<<" "<<((main_shape.container_shape.getter_polygon_points())[i].getter_pos_x())<<","<<((main_shape.container_shape.getter_polygon_points())[i].getter_pos_y());
		if(i<main_shape.container_shape.get_size()-1)
			outputfile<<" ";
	}
	outputfile<<"\""<<endl;
...
void composedShape::optimalFit(){
	...
	for(int i=1;((i*radius*2)+height_remaning)<=height;i++){
		for(int j=1;((j*radius*2)+(height_counter%2)*radius+(i%2)*width_remaning)<=width;++j){
			...
			if((j*radius*2+radius)<=width){
				...
				current_shape.setter_position_x((2*j-1)*radius);
				current_shape.setter_position_y((i-1)*(radius*pow(3,(1/2.0))));
				current_shape.setter_radius(radius);}
			}
			else{
				
				current_shape.setter_position_x(j*radius+(j-2)*radius);
				current_shape.setter_position_y((i-1)*(radius*pow(3,(1/2.0))));
				current_shape.setter_radius(radius);
			}
	...
	while((2*radius+(height_counter)*radius*pow(3,(1/2.0)))<=height){
		for(int j=1;((j*radius*2)+(height_counter%2)*radius+((height_counter+1)%2)*width_remaning)<=width;++j){
			...
			current_shape.setter_position_y((height_counter)*(radius*pow(3,(1/2.0))));
			current_shape.setter_radius(radius);
			}
			else{
			current_shape.setter_position_x(j*radius+(j-2)*radius);
	...
	for(int i=1;(i*radius*2)<=(height-height_remaning);i++){
		for(int j=1;(j*radius*2)<=(width-width_remaning);j++){
			...
			current_shape.setter_position_x(width_remaning/2+j*(radius)+(j-2)*radius);
			current_shape.setter_position_y(height_remaning/2+i*(radius)+(i-2)*radius);
			current_shape.setter_radius(radius);
...
/*likewise continues to lay shapes.*/
	...
	for(int i=1;i*shape_width<=(container_height-height_counter*shape_height);++i){
		...
		for(int j=1;(j*2)*radius_of_circle+(radius_of_circle*sqrt(3.0)-radius_of_circle)<=lenght_of_triangle-(radius_of_circle*sqrt(3.0)-radius_of_circle)-(i-1)*2*radius_of_circle;++j){
			...
			current_shape.setter_position_x((i-2)*radius_of_circle+radius_of_circle*sqrt(3.0)+(j*2-2)*radius_of_circle);
			current_shape.setter_position_y(lenght_of_triangle*sqrt(3.0)/2-(2*radius_of_circle+(i-1)*radius_of_circle*sqrt(3.0)));
	...
	for(int i=1;i*2*radius_of_small_circle<=radius_of_container_circle;++i){
		...
		current_shape.setter_position_x(radius_of_container_circle-radius_of_small_circle);
		current_shape.setter_position_y((2*i-2)*radius_of_small_circle);
		...
		*this+=a;
		...
		for(int j=1;((((j*2-1)*radius_of_small_circle+radius_of_small_circle*sqrt(3.0))<=sqrt(2*(radius_of_container_circle)*(2*i*radius_of_small_circle)-pow(2*i*radius_of_small_circle,2.0))) && (((j*2-2)*radius_of_small_circle+radius_of_small_circle*sqrt(3.0))+radius_of_small_circle/3<=sqrt(2*(radius_of_container_circle)*((2*i-1)*radius_of_small_circle)-pow((2*i-1)*radius_of_small_circle,2.0))));++j){
			...
			current_shapee.setter_position_x(radius_of_container_circle+radius_of_small_circle*sqrt(3.0)+(2*j-2)*radius_of_small_circle-radius_of_small_circle);
			current_shapee.setter_position_y((i*2-1)*radius_of_small_circle);
			...
			Polygon b(current_shapee);
			*this+=b;
	
			Circle current_shapee2;
			current_shapee2.plus_circles();
			
			current_shapee2.setter_position_x(radius_of_container_circle-(radius_of_small_circle*sqrt(3.0)+(2*j-2)*radius_of_small_circle)-radius_of_small_circle);
			current_shapee2.setter_position_y((i*2-1)*radius_of_small_circle);
	...
	for( ;height_counter*2*radius_of_small_circle<=2*radius_of_container_circle;++height_counter){
		...
		current_shape.setter_position_x(radius_of_container_circle-radius_of_small_circle);
		current_shape.setter_position_y((2*height_counter-2)*radius_of_small_circle);
		current_shape.setter_radius(radius_of_small_circle);
		...
		*this+=a;
		...
		for(int j=1;((((j*2-1)*radius_of_small_circle+radius_of_small_circle*sqrt(3.0))<=sqrt(2*(radius_of_container_circle)*(2*radius_of_container_circle-2*height_counter*radius_of_small_circle)-pow(2*radius_of_container_circle-2*height_counter*radius_of_small_circle,2.0))) &&  (((j*2-2)*radius_of_small_circle+radius_of_small_circle*sqrt(3.0))+radius_of_small_circle/3<=sqrt(2*(radius_of_container_circle)*(2*radius_of_container_circle-(2*height_counter+1)*radius_of_small_circle)-pow(2*radius_of_container_circle-(2*height_counter+1)*radius_of_small_circle,2.0))));++j){
			...
			current_shapee.setter_position_x(radius_of_container_circle+radius_of_small_circle*sqrt(3.0)+(2*j-2)*radius_of_small_circle-radius_of_small_circle);
			current_shapee.setter_position_y((height_counter*2-1)*radius_of_small_circle);
			current_shapee.setter_radius(radius_of_small_circle);
			...
			*this+=b;
			...
			current_shapee2.setter_position_x(radius_of_container_circle-(radius_of_small_circle*sqrt(3.0)+(2*j-2)*radius_of_small_circle)-radius_of_small_circle);
			current_shapee2.setter_position_y((height_counter*2-1)*radius_of_small_circle);
			current_shapee2.setter_radius(radius_of_small_circle);
